        ...
        <>
            ...
            {loading ?
                <div className="w-full h-screen flex justify-center items-center"><Oval visible={true} height="50"
                    ...
                    {times
                        .filter(({ informacoes }) => informacoes.jogadores.some(({ status }) => status === "0" || status === "1"))
                        .map((time, index) => <div className='flex justify-between text-center bg-card-white-1 w-full max-w-screen-sm md:max-w-screen-2xl rounded p-5' key={index}>
                            ...
                            <div className='p-2 md:text-base flex-1 md:p-5'>
                                {/* {time.informacoes.jogadores.map(({ status, id_time, id }) => (
                                    status === "0" ? (
                                        <div key={id_time} className='flex justify-center gap-4'>
                                            <button onClick={() => handleAceitarTime(id)} className='p-2 h-8 w-8 md:w-10 md:h-10 text-xs md:text-base text-center btn-confirm'>
                                                <FontAwesomeIcon icon={faCheck} /> {typeof (status)}</button>
                                            <button onClick={() => handleRejeitarTime(id)} className='p-2 h-8 w-8 md:w-10 md:h-10 text-xs md:text-base text-center btn-delete'>
